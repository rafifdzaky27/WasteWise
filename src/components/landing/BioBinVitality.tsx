...
const HealthIcon = <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" fill="#F59E0B"/></svg>;
...
const fallbackMetrics = [
  ...
  { value: "4 Hari", label: "Menuju Kompos", iconBg: "bg-purple-bg", iconBorder: "border-purple-border", icon: TimeIcon },
];

export default function BioBinVitality() {
  const sectionRef = useRef<HTMLElement>(null);
  const [displayMetrics, setDisplayMetrics] = useState(fallbackMetrics);

  useEffect(() => {
    const fetchSensorData = async () => {
      try {
        const supabase = createClient();
        // Fetch the latest sensor reading from sensor_readings table
        ...
        if (data && data.temperature != null && data.humidity != null) {
          setDisplayMetrics([
            { value: `${Math.round(data.temperature * 10) / 10}°C`, label: "Suhu", iconBg: "bg-accent-green", iconBorder: "border-accent-green-border", icon: TempIcon },
            { value: `${Math.round(data.humidity)}%`, label: "Kelembapan", iconBg: "bg-blue-bg", iconBorder: "border-blue-border", icon: HumidIcon },
            { value: "Sehat", label: "Kondisi", iconBg: "bg-yellow-bg", iconBorder: "border-yellow-border", icon: HealthIcon },
            { value: "4 Hari", label: "Menuju Kompos", iconBg: "bg-purple-bg", iconBorder: "border-purple-border", icon: TimeIcon },
          ]);
        }
        // If no data, fallbackMetrics stay (mockup)
      } catch {
